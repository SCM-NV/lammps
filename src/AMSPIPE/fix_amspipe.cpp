    ...
    Fix(lmp, narg, arg), irregular(new Irregular(lmp)), pipe(nullptr), exiting(false)
{
  ...
  // conversions from current LAMMPS units to atomic units used by AMSPipe
  // using force->boltz and force->angstrom constants from LAMMPS "electron" units
  potconv = 3.16681534e-06 / force->boltz;
  posconv = force->angstrom / 1.88972612;
